...
int main(int argc, char* argv[])
{
    ...
    VART::Transform instanciaBoneco;
    instanciaBoneco.MakeIdentity(); // Começa na origem, sem rotação
    scene.AddObject(&instanciaBoneco);

    Box torso;
    torso.MakeBox(-1, 1, -1.5, 1.5, -0.5, 0.5);
    torso.SetMaterial(VART::Material::DARK_PLASTIC_GRAY());
    instanciaBoneco.AddChild(torso);

    UniaxialJoint juntaPescoco;
    juntaPescoco.AddDof(Point4D::Y(), Point4D(0,1.5,0), -1.0, 1.0);
    torso.AddChild(juntaPescoco);

    Box cabeca;
    cabeca.MakeBox(-0.8, 0.8, -0.8, 0.8, -0.8, 0.8);
    cabeca.SetMaterial(VART::Material::PLASTIC_BLUE());
    juntaPescoco.AddChild(cabeca);

    UniaxialJoint juntaBracoD;
    juntaBracoD.AddDof(Point4D::Z(), Point4D(1,1,0), -1.5, 1.5);
    torso.AddChild(juntaBracoD);

    Box bracoD;
    bracoD.MakeBox(1, 2.5, -0.3, 0.3, -0.3, 0.3);
    bracoD.SetMaterial(VART::Material::PLASTIC_RED());
    juntaBracoD.AddChild(bracoD);

    Box maoD;
    maoD.MakeBox(-0.3, 0.3, -0.5, 0, -0.3, 0.3);
    maoD.SetMaterial(VART::Material::PLASTIC_RED());
    bracoD.AddChild(maoD);

    UniaxialJoint juntaBracoE;
    juntaBracoE.AddDof(Point4D::Z(), Point4D(-1,1,0), -1.5, 1.5);
    torso.AddChild(juntaBracoE);

    Box bracoE;
    bracoE.MakeBox(-2.5, -1, -0.3, 0.3, -0.3, 0.3);
    bracoE.SetMaterial(VART::Material::PLASTIC_RED());
    juntaBracoE.AddChild(bracoE);

    Box maoE;
    maoE.MakeBox(-0.3, 0.3, -0.5, 0, -0.3, 0.3);
    maoE.SetMaterial(VART::Material::PLASTIC_RED());
    bracoE.AddChild(maoE);

    UniaxialJoint juntaPernaD;
    juntaPernaD.AddDof(Point4D::Z(), Point4D(0.5,-1.5,0), -1.5, 1.5);
    torso.AddChild(juntaPernaD);

    Box pernaD;
    pernaD.MakeBox(0.3, 1.3, -0.3, 0.3, -0.3, 0.3);
    pernaD.SetMaterial(VART::Material::PLASTIC_RED());
    juntaPernaD.AddChild(pernaD);

    Box peD;
    peD.MakeBox(-0.5, 0.5, -1.5, -1.0, -0.3, 0.3);
    peD.SetMaterial(VART::Material::PLASTIC_RED());
    pernaD.AddChild(peD);   

    UniaxialJoint juntaPernaE;
    juntaPernaE.AddDof(Point4D::Z(), Point4D(-0.5,-1.5,0), -1.5, 1.5);
    torso.AddChild(juntaPernaE);
    
    Box pernaE;
    pernaE.MakeBox(-1.3, -0.3, -0.3, 0.3, -0.3, 0.3);
    pernaE.SetMaterial(VART::Material::PLASTIC_RED());
    juntaPernaE.AddChild(pernaE);

    Box peE;
    peE.MakeBox(-0.5, 0.5, -1.5, -1.0, -0.3, 0.3);  
    peE.SetMaterial(VART::Material::PLASTIC_RED());
    pernaE.AddChild(peE);
