...
#include <cmath> // Necessário para a função sin()

class MyIHClass : public VART::ViewerGlutOGL::IdleHandler
{
    public:
        MyIHClass() : contadorTempo(0) {}
        virtual ~MyIHClass() {}
        virtual void OnIdle() {
            ...
            // 2. Calcula a escala variável (Seno faz ir e voltar)
            // Varia de 0.5 a 2.0 (aproximadamente)
            float escala = 1.25 + (sin(contadorTempo) * 0.75);
            
            // 3. Aplica a escala
            if (boxTransform) {
                boxTransform->MakeScale(escala, escala, escala);
            }
        ...
        // Ponteiro para a transformação do cubo
        VART::Transform* boxTransform;
...
int main(int argc, char* argv[])
{
    ...
    Transform boxScale;
    scene.AddObject(&boxScale);

    Box box;
    box.MakeBox(-0.5,0.5,-0.5,0.5,-0.5,0.5);
    box.SetMaterial(VART::Material::DARK_PLASTIC_GRAY());

    boxScale.AddChild(box);

    MyIHClass idleHandler;
    idleHandler.boxTransform = &boxScale;
